...
describe('canvas_from_gl', () => {
  test('should map points as expected', () => {
    ...
    expect(apply(gl_from_canvas, { x: 800, y: 0 })).toEqual({ x: 1, y: 1 });

    expect(apply(gl_from_canvas, { x: 0, y: 600 })).toEqual({ x: -1, y: -1 });
    expect(apply(gl_from_canvas, { x: 800, y: 600 })).toEqual({ x: 1, y: -1 });
